"""
Transcript Rationale API Endpoints
8-step pipeline for YouTube transcript analysis focusing on stock detection
"""
...
transcript_rationale_bp = Blueprint('transcript_rationale', __name__, url_prefix='/api/v1/transcript-rationale')

TRANSCRIPT_STEPS = [
    {"step_number": 1, "name": "Translate", "description": "Translate transcript to English"},
    {"step_number": 2, "name": "Detect Stocks", "description": "Detect stocks discussed by analyst"},
    {"step_number": 3, "name": "Web Search Symbols", "description": "Search for correct NSE symbols"},
    {"step_number": 4, "name": "Map Master File", "description": "Map stocks to master data"},
    {"step_number": 5, "name": "Extract Analysis", "description": "Extract and polish analysis with chart type"},
    {"step_number": 6, "name": "Fetch CMP", "description": "Fetch current market prices"},
    {"step_number": 7, "name": "Generate Charts", "description": "Generate stock charts"},
    {"step_number": 8, "name": "Generate PDF", "description": "Create final PDF report"},
]


def run_transcript_pipeline(job_id, job_folder, call_date, call_time, start_step=1, end_step=None):
    """Run the transcript rationale pipeline in background
    ...
    """
    from backend.pipeline.transcript import (
        step01_translate,
        step02_detect_stocks,
        step03_web_search_symbols,
        step04_map_master,
        step05_extract_analysis,
        step06_fetch_cmp,
        step07_generate_charts,
        step08_generate_pdf
    )
    
    steps = [
        (1, "Translate", step01_translate.run, [job_folder]),
        (2, "Detect Stocks", step02_detect_stocks.run, [job_folder]),
        (3, "Web Search Symbols", step03_web_search_symbols.run, [job_folder]),
        (4, "Map Master File", step04_map_master.run, [job_folder]),
        (5, "Extract Analysis", step05_extract_analysis.run, [job_folder]),
        (6, "Fetch CMP", step06_fetch_cmp.run, [job_folder]),
        (7, "Generate Charts", step07_generate_charts.run, [job_folder, call_date, call_time]),
        (8, "Generate PDF", step08_generate_pdf.run, [job_folder]),
    ]
    ...
    try:
        for step_num, step_name, step_func, step_args in steps:
            ...
            with get_db_cursor(commit=True) as cursor:
                ...
                cursor.execute("""
                    ...
                    WHERE id = %s
                """, (step_num, int((step_num - 1) / 8 * 100), datetime.now(), job_id))
            ...
            if result.get('success'):
                ...
                # After Step 5 completes, pause for CSV review
                if step_num == 5:
                    with get_db_cursor(commit=True) as cursor:
                        cursor.execute("""
                            UPDATE jobs 
                            SET status = 'awaiting_step5_review', progress = 62, updated_at = %s
                            WHERE id = %s
                        """, (datetime.now(), job_id))
                    print(f"Job {job_id}: Paused after Step 5 for analysis CSV review")
                    return
                
                # After Step 7 (Generate Charts), check for failed charts
                if step_num == 7:
                    ...
                    if failed_charts:
                        ...
                        with get_db_cursor(commit=True) as cursor:
                            cursor.execute("""
                                UPDATE jobs 
                                SET status = 'awaiting_chart_upload', progress = 87, updated_at = %s
                                WHERE id = %s
                            """, (datetime.now(), job_id))
                        ...
                        print(f"Job {job_id}: Paused after Step 7 - {len(failed_charts)} chart(s) need manual upload")
                        return
            else:
                with get_db_cursor(commit=True) as cursor:
                    cursor.execute("""
        ...
        if end_step is None or end_step >= 8:
            with get_db_cursor(commit=True) as cursor:
                cursor.execute("""
                    UPDATE jobs 
                    SET status = 'pdf_ready', progress = 100, current_step = 8, updated_at = %s
                    WHERE id = %s
                """, (datetime.now(), job_id))
            
            print(f"\n✅ Transcript Rationale pipeline completed for job {job_id}")
...
@transcript_rationale_bp.route('/create-job', methods=['POST'])
@jwt_required()
def create_job():
    """Create a new transcript rationale job"""
    try:
        ...
        if not channel_id or not call_date or not input_text.strip():
            return jsonify({'error': 'Channel, date, and transcript text are required'}), 400
        ...
        with get_db_cursor(commit=True) as cursor:
            ...
            job_id = f"transcript-{uuid.uuid4().hex[:8]}"
            ...
            input_file_path = os.path.join(job_folder, 'transcript-input.txt')
            ...
            """, (
                ...
                current_user_id, 'Transcript Rationale', 'processing', 0, 0, job_folder,
            ...
            create_activity_log(
                current_user_id,
                'job_started',
                f'Started Transcript Rationale: {title}',
                job_id,
                'Transcript Rationale'
            )
        
        thread = threading.Thread(
            target=run_transcript_pipeline,
        ...
        return jsonify({
            ...
            'jobId': job_id,
            'title': title,
            'message': 'Transcript Rationale job started'
        }), 200
...
@transcript_rationale_bp.route('/jobs/<job_id>', methods=['GET'])
@jwt_required()
def get_job(job_id):
    """Get transcript rationale job status and details"""
    try:
        current_user_id = get_jwt_identity()
...
@transcript_rationale_bp.route('/jobs/<job_id>', methods=['DELETE'])
@jwt_required()
def delete_job(job_id):
    """Delete a transcript rationale job"""
    try:
        current_user_id = get_jwt_identity()
...
@transcript_rationale_bp.route('/jobs/<job_id>/save', methods=['POST'])
@jwt_required()
def save_job(job_id):
    """Save transcript rationale job to saved_rationale table"""
    try:
        current_user_id = get_jwt_identity()
        ...
        with get_db_cursor(commit=True) as cursor:
            ...
            pdf_path = os.path.join(job['folder_path'], 'pdf', 'transcript_rationale.pdf')
            ...
            cursor.execute("""
                ...
                RETURNING id
            """, (
                job_id, 'Transcript Rationale', job['channel_id'], job['title'],
            ...
            create_activity_log(
                current_user_id,
                'job_completed',
                f'Saved Transcript Rationale: {job["title"]}',
                job_id,
                'Transcript Rationale'
            )
...
@transcript_rationale_bp.route('/jobs/<job_id>/download', methods=['GET'])
@jwt_required()
...
@transcript_rationale_bp.route('/jobs/<job_id>/upload-signed', methods=['POST'])
@jwt_required()
def upload_signed_pdf(job_id):
    ...
    try:
        ...
        with get_db_cursor(commit=True) as cursor:
            ...
            signed_filename = f'transcript_rationale_signed.pdf'
...
@transcript_rationale_bp.route('/restart-step/<job_id>/<int:step_number>', methods=['POST'])
@jwt_required()
...
# ==================== Step 5 CSV Review Endpoints ====================

@transcript_rationale_bp.route('/jobs/<job_id>/step5-csv-preview', methods=['GET'])
@jwt_required()
def get_step5_csv_preview(job_id):
    """Get stocks_with_analysis.csv content for preview after Step 5"""
    try:
        current_user_id = get_jwt_identity()
        ...
        if not csv_path or not os.path.exists(csv_path):
            return jsonify({'error': 'stocks_with_analysis.csv not found'}), 404
        ...
        print(f"Error getting Step 5 CSV preview: {str(e)}")
        return jsonify({'error': f'Failed to get CSV preview: {str(e)}'}), 500


@transcript_rationale_bp.route('/jobs/<job_id>/step5-download-csv', methods=['GET'])
@jwt_required()
def download_step5_csv(job_id):
    ...
    try:
        ...
        print(f"Error downloading Step 5 CSV: {str(e)}")
        return jsonify({'error': f'Failed to download CSV: {str(e)}'}), 500


@transcript_rationale_bp.route('/jobs/<job_id>/step5-upload-csv', methods=['POST'])
@jwt_required()
def upload_step5_csv(job_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'stocks_with_analysis.csv uploaded and replaced successfully'
        }), 200
        
    except Exception as e:
        print(f"Error uploading Step 5 CSV: {str(e)}")
        return jsonify({'error': f'Failed to upload CSV: {str(e)}'}), 500


@transcript_rationale_bp.route('/jobs/<job_id>/step5-continue-pipeline', methods=['POST'])
@jwt_required()
def step5_continue_pipeline(job_id):
    """Continue pipeline execution from Step 6 after Step 5 CSV review"""
    try:
        current_user_id = get_jwt_identity()
        ...
        with get_db_cursor() as cursor:
            ...
            if job['status'] != 'awaiting_step5_review':
                return jsonify({'error': 'Job is not in awaiting_step5_review status'}), 400
        
        with get_db_cursor(commit=True) as cursor:
            cursor.execute("""
                UPDATE jobs 
                SET status = 'processing', current_step = 6, updated_at = %s
                WHERE id = %s
            """, (datetime.now(), job_id))
        ...
        def run_remaining_steps():
            try:
                ...
                print(f"Error continuing pipeline from Step 6 for job {job_id}: {str(e)}")
                with get_db_cursor(commit=True) as cursor:
                    cursor.execute("""
                        UPDATE jobs 
        ...
        return jsonify({
            'success': True,
            'message': 'Pipeline continuing from Step 6'
        }), 200
        
    except Exception as e:
        print(f"Error continuing pipeline from Step 5: {str(e)}")
        return jsonify({'error': str(e)}), 500


@transcript_rationale_bp.route('/jobs/<job_id>/step5-save-edits', methods=['POST'])
@jwt_required()
def save_step5_edits(job_id):
    """Save inline edits to the Step 5 CSV file"""
    try:
        current_user_id = get_jwt_identity()
        ...
        print(f"✅ Step 5 CSV saved with {len(csv_data)} rows for job {job_id}")
        ...
        print(f"Error saving Step 5 edits: {str(e)}")
        import traceback
        traceback.print_exc()
...
@transcript_rationale_bp.route('/jobs/<job_id>/failed-charts', methods=['GET'])
@jwt_required()
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            cursor.execute("""
                ...
                WHERE job_id = %s AND step_number = 7
            """, (job_id,))
...
@transcript_rationale_bp.route('/jobs/<job_id>/upload-chart/<int:stock_index>', methods=['POST'])
@jwt_required()
...
@transcript_rationale_bp.route('/jobs/<job_id>/step7-continue-pipeline', methods=['POST'])
@jwt_required()
def step7_continue_pipeline(job_id):
    """Continue pipeline execution from Step 8 (PDF) after chart uploads"""
    try:
        current_user_id = get_jwt_identity()
        ...
        with get_db_cursor(commit=True) as cursor:
            cursor.execute("""
                UPDATE jobs 
                SET status = 'processing', current_step = 8, updated_at = %s
                WHERE id = %s
            """, (datetime.now(), job_id))
...
@transcript_rationale_bp.route('/jobs/<job_id>/skip-failed-charts', methods=['POST'])
@jwt_required()
