...
RULES:
1. Find the EXACT NSE trading symbol
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert on Indian stock markets. You have extensive knowledge of NSE stock symbols. Always return the exact trading symbol without any suffixes."
                },
                {
                    "role": "user",
                    "content": prompt
        ...
        result = result.replace('.NS', '').replace('.NSE', '').replace('.BO', '').replace('.BSE', '')
...
def run(job_folder):
    ...
    try:
        input_file = os.path.join(job_folder, 'analysis', 'detected_stocks.csv')
